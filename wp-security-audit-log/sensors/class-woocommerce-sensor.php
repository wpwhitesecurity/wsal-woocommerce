...
if ( ! class_exists( '\WSAL\Plugin_Sensors\WooCommerce_Sensor' ) ) {
	...
	class WooCommerce_Sensor {
		 ...
		 * @var array
		 */
		private static $_old_product_attributes = null;
		...
		public static function orders_actions() {
			...
			if ( isset( $_GET['action'] ) ) {
				if ( 'edit' === $_GET['action'] && isset( $_GET['id'] ) && 0 < (int) $_GET['id'] ) {
					// Get post.
					$post = wc_get_order( \wp_unslash( \sanitize_text_field( $_GET['id'] ) ) );
		...
		public static function check_product_changes_after_save( $product ) {
			...
			if ( ! isset( $_POST['action'] ) ) {
				...
				$old_product_data = self::$_old_data;
				...
				foreach ( $values_to_lookup as $lookup_key ) {
					if ( isset( self::$new_data[ $lookup_key ] ) && $old_product_data[ $lookup_key ] !== self::$new_data[ $lookup_key ] || isset( self::$_old_data[ $lookup_key ] ) && self::$new_data[ $lookup_key ] !== $old_product_data[ $lookup_key ] ) {
						if ( 'regular_price' === $lookup_key || 'sale_price' === $lookup_key ) {
							self::check_price_change( self::$_old_post );
						} elseif ( 'stock_status' === $lookup_key ) {
							self::check_stock_status_change( self::$_old_post );
						} elseif ( 'stock' === $lookup_key ) {
							self::check_stock_quantity_change( self::$_old_post );
						} elseif ( 'sku' === $lookup_key ) {
							self::check_sku_change( self::$_old_post );
						} elseif ( 'weight' === $lookup_key ) {
							self::check_weight_change( self::$_old_post );
						} elseif ( 'tax_status' === $lookup_key ) {
							self::check_tax_status_change( self::$_old_post, self::$_old_meta_data, self::$new_data );
						}
					}
				}
			}
		...
		public static function inline_product_changed( $product ) {
			...
			self::event_changed( $product->get_id(), get_post( $product->get_id() ), true );
		}
		...
		public static function get_before_post_edit_data( $post_id ) {
			...
			if ( ! empty( $post ) && $post instanceof \WP_Post && in_array( $post->post_type, array( 'product', 'shop_order', 'shop_coupon' ), true ) ) {
				...
				self::$_old_cats                = 'product' === $post->post_type ? self::get_product_categories( self::$_old_post ) : null;
				self::$_old_data                = 'product' === $post->post_type ? self::get_product_data( self::$old_product ) : null;
				self::$_old_product_attributes  = get_post_meta( $post->ID, '_product_attributes' );
		 ...
		 */
		public static function event_changed( $post_id, $post, $update ) {
			if ( ! $update ) {
			...
			if ( 'product' === $post->post_type ) {
				if (
					( isset( self::$_old_post ) && 'auto-draft' === self::$_old_post->post_status && 'draft' === $post->post_status ) // Saving draft.
					...
					$changes = 0;
					$changes = self::check_categories_change( self::$_old_cats, self::get_product_categories( $post ), self::$_old_post, $post )
						+ self::check_short_description_change( self::$_old_post, $post )
						+ self::check_text_change( self::$_old_post, $post )
						+ self::check_date_change( self::$_old_post, $post )
						+ self::check_visibility_change( self::$_old_post, $post )
						+ self::check_status_change( self::$_old_post, $post )
						+ self::check_title_change( self::$_old_post, $post )
						+ self::check_product_type_change( self::$_old_post )
						+ self::check_catalog_visibility_change( self::$_old_post )
						+ self::check_featured_product( self::$_old_post )
						+ self::check_price_change( self::$_old_post )
						+ self::check_sku_change( self::$_old_post )
						+ self::check_stock_status_change( self::$_old_post )
						+ self::check_stock_quantity_change( self::$_old_post )
						+ self::check_type_change( self::$_old_post, $post )
						+ self::check_weight_change( self::$_old_post )
						+ self::check_dimensions_change( self::$_old_post )
						+ self::check_downloadable_file_change( self::$_old_post )
						+ self::check_backorders_setting( self::$_old_post )
						+ self::check_upsells_change( self::$_old_post )
						+ self::check_cross_sell_change( self::$_old_post )
						+ self::check_attributes_change( self::$_old_post )
						+ self::check_image_change( self::$_old_post )
						+ self::check_download_limit_change( self::$_old_meta_data )
						+ self::check_tax_status_change( self::$_old_post, self::$_old_meta_data, self::$new_data )
						+ self::check_low_stock_threshold_change( self::$_old_post, self::$_old_meta_data, self::$new_data );

					if ( ! $changes ) {
						// Change Permalink.
						$changes = self::check_permalink_change( self::$_old_link, get_post_permalink( $post_id, false, true ), $post );
						if ( ! $changes ) {
							// If no one of the above changes happen.
							self::check_modify_change( self::$_old_post, $post );
						}
					}
				}
			} elseif ( 'shop_order' === $post->post_type ) {
				// Check order events.
				self::check_order_modify_change( $post_id, self::$_old_post, $post );
			} elseif ( 'shop_coupon' === $post->post_type ) {
				// Check coupon events.
				$changes = 0 + self::event_creation( self::$_old_post, $post );
		...
		private static function event_creation( $old_post, $new_post ) {
			...
			if ( 'product' === $old_post->post_type ) {
				...
				Alert_Manager::trigger_event( 9063, $coupon_data );
				return 1;
			}
			return 0;
		}
			...
			if ( ! empty( $term ) ) {
				Alert_Manager::trigger_event(
					...
					array(
						...
						'ProductCatLink' => self::get_taxonomy_edit_link( $term_id ),
					)
				);
			}
		}
			...
			if ( ! empty( $term ) ) {
				Alert_Manager::trigger_event(
					...
					array(
						'CategoryName'   => sanitize_text_field( $term->name ),
						'Slug'           => sanitize_text_field( $term->slug ),
						'ProductTagLink' => self::get_taxonomy_edit_link( $term_id, 'product_tag' ),
					)
				);
			}
		}
		...
		private static function check_product_type_change( $post ) {
			$old_type = isset( self::$_old_data['type'] ) ? self::$_old_data['type'] : false;
		...
		private static function check_catalog_visibility_change( $post ) {
			// Get product data.
			$old_visibility = isset( self::$_old_data['catalog_visibility'] ) ? self::$_old_data['catalog_visibility'] : false;
		...
		private static function check_featured_product( $post ) {
			$old_featured = isset( self::$_old_data['featured'] ) ? self::$_old_data['featured'] : false;
		...
		private static function check_backorders_setting( $oldpost, $old_backorder = '', $new_backorder = '' ) {
			// Get product data.
			if ( '' === $old_backorder ) {
				$old_backorder = isset( self::$_old_data['backorders'] ) ? self::$_old_data['backorders'] : false;
			}
			if ( '' === $new_backorder ) {
		...
		private static function check_upsells_change( $oldpost ) {
			// Get product data.
			$old_upsell_ids = isset( self::$_old_data['upsell_ids'] ) ? self::$_old_data['upsell_ids'] : false;
		...
		private static function check_cross_sell_change( $oldpost ) {
			// Get product data.
			$old_cross_sell_ids = isset( self::$_old_data['cross_sell_ids'] ) ? self::$_old_data['cross_sell_ids'] : false;
		...
		private static function check_price_change( $post ) {
			$result         = 0;
			$old_price      = isset( self::$_old_data['regular_price'] ) ? self::$_old_data['regular_price'] : false;
			$old_sale_price = isset( self::$_old_data['sale_price'] ) ? self::$_old_data['sale_price'] : false;
		...
		private static function check_sku_change( $oldpost, $old_sku = '', $new_sku = '' ) {
			if ( '' === $old_sku && '' === $new_sku ) {
				$old_sku = isset( self::$_old_data['sku'] ) ? self::$_old_data['sku'] : false;
		...
		private static function check_stock_status_change( $oldpost, $old_status = '', $new_status = '' ) {
			if ( '' === $old_status && '' === $new_status ) {
				$old_status = isset( self::$_old_data['stock_status'] ) ? self::$_old_data['stock_status'] : false;
		...
		private static function check_stock_quantity_change( $oldpost, $old_value = false, $new_value = false ) {
			if ( false === $old_value && false === $new_value ) {
				$old_value = isset( self::$_old_data['stock_quantity'] ) ? self::$_old_data['stock_quantity'] : false;
		...
		private static function check_type_change( $oldpost, $newpost = null, $virtual = false, $download = false ) {
			...
			if ( false === $virtual ) {
				$old_virtual = isset( self::$_old_data['virtual'] ) ? self::$_old_data['virtual'] : false;
			...
			if ( false === $download ) {
				$old_download = isset( self::$_old_data['downloadable'] ) ? self::$_old_data['downloadable'] : false;
		...
		private static function check_weight_change( $oldpost, $old_weight = '', $new_weight = '' ) {
			if ( '' === $old_weight && '' === $new_weight ) {
				$old_weight = isset( self::$_old_data['weight'] ) ? self::$_old_data['weight'] : false;
		...
		private static function check_dimensions_change( $oldpost, $length = false, $width = false, $height = false ) {
			...
			// Length.
			if ( false === $length ) {
				$old_length = isset( self::$_old_data['length'] ) ? self::$_old_data['length'] : false;
			...
			// Width.
			if ( false === $width ) {
				$old_width = isset( self::$_old_data['width'] ) ? self::$_old_data['width'] : false;
			...
			// Height.
			if ( false === $height ) {
				$old_height = isset( self::$_old_data['height'] ) ? self::$_old_data['height'] : false;
		...
		private static function check_downloadable_file_change( $oldpost, $file_names = false, $file_urls = false ) {
			...
			if ( false === $file_names ) {
				$old_file_names = isset( self::$_old_data['file_names'] ) ? self::$_old_data['file_names'] : array();
			...
			if ( false === $file_urls ) {
				$old_file_urls = isset( self::$_old_data['file_urls'] ) ? self::$_old_data['file_urls'] : array();
		...
		private static function was_triggered( $alert_id ) {
			...
			$query->set_limit( 1 );
			$last_occurence = $query->get_adapter()->execute_query( $query );
		...
		public static function event_order_items_added( $item_id, $item, $order_id ) {
			...
			if ( $item instanceof \WC_Order_Item_Product ) {
				...
				error_log( print_r( 'event_order_items_added ALT', true ) );
				...
				if ( $product instanceof \WC_Product ) {
					...
					error_log( print_r( self::ignore_if_new_order(), true ) );
		...
		public static function event_order_items_removed( $item_id ) {
			...
			if ( isset( $order->get_items()[ $item_id ] ) ) {
				...
				error_log( print_r( 'event_order_items_removed ALT', true ) );
				error_log( print_r( '$item_id', true ) );
				error_log( print_r( $item_id, true ) );
		 ...
		 * @since 3.3.1
		 */
		private static function check_wc_ajax_change_events() {
			// @codingStandardsIgnoreStart
		...
		private static function check_attributes_change( $oldpost, $data = false ) {
			...
			if ( isset( self::$_old_product_attributes[0] ) ) {
				$old_attributes = self::$_old_product_attributes[0];
			} else {
						...
						if ( $added_attribute && ! empty( $added_attribute['name'] ) && ! empty( $added_attribute['value'] ) ) {
							Alert_Manager::trigger_event(
								...
								array(
									...
									'SKU'                => self::get_product_sku( $oldpost->ID ),
									'ProductTitle'       => sanitize_text_field( $oldpost->post_title ),
									'ProductStatus'      => sanitize_text_field( $oldpost->post_status ),
									$editor_link['name'] => $editor_link['value'],
								)
		...
		private static function check_download_limit_change( $oldpost, $data = false ) {
			...
			$event_id     = false;
			$editor_link  = self::get_editor_link( self::$_old_post );
			$alert_needed = false;
			...
			if ( $meta_value !== $old_display ) {
				Alert_Manager::trigger_event(
					...
					array(
						...
						'ProductCatLink' => self::get_taxonomy_edit_link( $object_id ),
					)
				);
			}
		}
		...
		public static function log_coupon_meta_update_events( $log_meta_event, $meta_key, $meta_value, $old_meta_obj, $coupon ) {
			...
			if ( in_array( $meta_key, $ignore_coupon_meta, true ) && $meta_value !== $old_meta_obj->val ) {
				...
				if ( 'discount_type' === $meta_key ) {
					...
					$coupon_data['OldDate'] = isset( $old_meta_obj->val ) && ! empty( $old_meta_obj->val ) ? gmdate( get_option( 'date_format' ), $old_meta_obj->val ) : __( 'Does not expire', 'wsal-woocommerce' );
					$coupon_data['NewDate'] = ! empty( $meta_value ) ? gmdate( get_option( 'date_format' ), $meta_value ) : __( 'Does not expire', 'wsal-woocommerce' );
				...
				if ( $event_id && ! empty( $coupon_data ) ) {
					// Log the event.
					Alert_Manager::trigger_event_if( $event_id, $coupon_data, array( __CLASS__, 'must_not_be_new_coupon' ) );
				}
			}
			return false;
		}
		...
		public static function check_product_changes_before_save( $product, $data_store ) {
			...
			if ( ! isset( $_POST['action'] ) ) {
				...
				self::$_old_post      = get_post( $product_id );
				self::$_old_data      = self::get_product_data( self::$old_product );
		 ...
		 */
		private static function get_taxonomy_edit_link( $tag_id, $taxonomy = 'product_cat' ) {
			$tag_args = array(
				'post_type' => 'product',
				'taxonomy'  => $taxonomy,
				'tag_ID'    => $tag_id,
		 ...
		 */
		private static function create_webhook_editor_link( $webhook_id ) {
			$editor_link = esc_url(
				add_query_arg(
					array(
						'tab'          => 'advanced',
